...
class JsonToExcel {
  constructor(options = {}) {
    ...
    if (typeof options.jsonFilePath !== 'string' || options.jsonFilePath.trim() === '') {
      throw new Error('Invalid or missing JSON file path.');
    }
    this.jsonFilePath = options.jsonFilePath;
    
    // Check if outputFileName is provided and it's a string
    if (typeof options.outputFileName !== 'string' || options.outputFileName.trim() === '') {
